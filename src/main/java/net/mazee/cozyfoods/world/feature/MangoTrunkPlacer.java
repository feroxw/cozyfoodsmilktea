...
public class MangoTrunkPlacer extends TrunkPlacer {
    ...
    public MangoTrunkPlacer(int baseHeight, int height_rand_a, int height_rand_b) {
        super(baseHeight, height_rand_a, height_rand_b);
    }

    public MangoTrunkPlacer(int baseHeight, int height_rand_a, int height_rand_b, boolean isWorldGen) {
    ...
    protected TrunkPlacerType<?> type() {
        return ModTrunkPlacers.MANGO_TRUNK_PLACER.get();
    }

    public static final Codec<MangoTrunkPlacer> CODEC = RecordCodecBuilder.create(builder ->
            builder.group(Codec.intRange(0, 32).fieldOf("base_height").forGetter(placer -> placer.baseHeight),
                    ...
                    .apply(builder, MangoTrunkPlacer::new));
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader world, BiConsumer<BlockPos, BlockState> consumer, RandomSource rand, int foliageHeight, BlockPos pos, TreeConfiguration baseTreeFeatureConfig) {
        ...
        int numBranches = 0;
        int lastBranch = 0;
        double branchChance = 0.4;
        int northB = 3 + rand.nextInt(foliageHeight - 5);
        int southB = 3 + rand.nextInt(foliageHeight - 5);
        int eastB = 3 + rand.nextInt(foliageHeight - 5);
        int westB = 3 + rand.nextInt(foliageHeight - 5);
    ...
    public void addBranch(LevelSimulatedReader world, BlockPos pos, int height, Direction d, RandomSource random, TreeConfiguration baseTreeFeatureConfig, BiConsumer<BlockPos, BlockState> consumer) {
        ...
        addLineDown(world, pos.relative(d, 4).relative(left, 1), d, 3, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 4).relative(right, 1), d, 1, random, baseTreeFeatureConfig, 0.3f, consumer);

        addLineDown(world, pos.relative(d, 5).above(1), d, 4, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 5).relative(left, 1).above(1), d, 2, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 5).relative(right, 1).above(1), d, 3, random, baseTreeFeatureConfig, 0.3f, consumer);

        //Side of branch hanging
        addLineDown(world, pos.relative(d, 2).relative(left, 2), d, 1, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 3).relative(left, 2), d, 2, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 2).relative(right, 2), d, 1, random, baseTreeFeatureConfig, 0.3f, consumer);
        addLineDown(world, pos.relative(d, 3).relative(right, 2), d, 3, random, baseTreeFeatureConfig, 0.3f, consumer);
    }
