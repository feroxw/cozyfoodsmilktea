...
public class ChairEntity extends Entity {
    ...
    {
        super(ModEntities.CHAIR.get(), level);
        this.noPhysics = true;
    }
    ...
    public static InteractionResult create(Level level, BlockPos pos, double yOffset, Player player, Direction direction)
    {
        ...
        {
            List<ChairEntity> chairs = level.getEntitiesOfClass(ChairEntity.class, new AABB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0, pos.getY() + 1.0, pos.getZ() + 1.0));
            if(chairs.isEmpty())
            {
                ChairEntity chair = new ChairEntity(level, pos, yOffset, direction.getOpposite());
